  ...
  return (
    ...
    >
      ...
      <DossierCard className="p-6 mb-6" hover={false}>
        <div className="flex items-center justify-between mb-5 pb-3 border-b-2 border-ink/80">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="label-mono-xs text-ink-mute">· QUEEN OF HEARTS · 41 UNITS</span>
        ...
        <div className="grid lg:grid-cols-[1fr_2fr_1fr] gap-6">
          {/* Cost breakdown */}
          <div>
            <div className="label-mono text-hazard mb-3">▼ COST BREAKDOWN</div>
            <div className="space-y-2 typewriter text-[13px]">
          ...
          {/* Per container */}
          <div>
            <div className="label-mono text-hazard mb-3">▼ PER CONTAINER</div>
            <div className="grid sm:grid-cols-2 gap-3">
              {containers.map((c) => (
                <div key={c.name} className="border-2 border-ink/30 p-3 bg-paper-deep/20">
                  <div className="flex justify-between items-baseline mb-2">
                    <div className="font-display text-sm">(2) {c.name}</div>
                    <div className={`label-mono font-bold ${c.color === "amber" ? "text-amber-700" : "text-ink"}`}>{c.profit}</div>
                  </div>
                  <div className="grid grid-cols-3 gap-1 typewriter text-[11px]">
                    <div><div className="label-mono-xs text-ink-mute">SELL</div><div>{c.sell}</div></div>
                    <div><div className="label-mono-xs text-ink-mute">COST</div><div>{c.cost}</div></div>
                    <div><div className="label-mono-xs text-ink-mute">PROFIT</div><div className="text-emerald-700">{c.p}</div></div>
                  </div>
          ...
          <div className="bg-ink text-paper p-5 border-2 border-ink relative">
            ...
            <div className="label-mono-xs text-paper/70 mb-1 mt-2">TOTAL BATCH OVERALL</div>
            <div className="font-display text-6xl text-hazard leading-none mb-1">67.7<span className="text-2xl">%</span></div>
            <div className="typewriter text-[12px] text-paper/80 mb-4">Strong — premium positioning. Industry: 50–65%</div>
            <div className="space-y-1.5 pt-4 border-t border-paper/30">
              <div className="flex justify-between label-mono"><span className="text-paper/60">REVENUE</span><span>$445.95</span></div>
              <div className="flex justify-between label-mono"><span className="text-paper/60">COST</span><span>$143.93</span></div>
              <div className="flex justify-between label-mono"><span className="text-hazard">PROFIT</span><span className="text-hazard">$302.02</span></div>
            </div>
          </div>
        </div>
      </DossierCard>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        {/* Recipe */}
        <DossierCard className="p-5">
          <div className="label-mono text-hazard mb-2">▼ RECIPE</div>
          <h3 className="font-display text-xl mb-3">QUEEN OF HEARTS</h3>
          <select className="w-full border-2 border-ink/40 bg-paper px-3 btn-min label-mono mb-3 hover:border-ink">
            <option>QUEEN OF HEARTS ▾</option>
          </select>
          <div className="space-y-1.5 typewriter text-[13px]">
            <div><span className="label-mono-xs text-ink-mute mr-2">WAX:</span>Unknown</div>
            <div><span className="label-mono-xs text-ink-mute mr-2">LOAD:</span>12% (from settings)</div>
            <div><span className="label-mono-xs text-ink-mute mr-2">OILS:</span>Cherry Eclipse, Leather</div>
          </div>
          <div className="mt-4 pt-3 border-t border-dashed border-ink/30 flex items-center gap-2 text-hazard typewriter text-[12px]">
            <AlertTriangle className="w-3.5 h-3.5" /> Not enough stock — 21.8 oz short
          </div>
        </DossierCard>
        ...
        <DossierCard className="p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              <div className="label-mono text-hazard">▼ CONTAINERS</div>
              <h3 className="font-display text-xl">UNITS</h3>
            </div>
            <button className="label-mono border-2 border-ink/30 btn-min-sm px-3 flex items-center gap-1 hover:border-ink">
          ...
          {[{ s: "48oz CANDLE", p: "48 oz · $12", a: 2 }, { s: "16oz CANDLE", p: "14 oz · $1", a: 2 }].map((c) => (
            <div key={c.s} className="mb-4">
              <div className="flex justify-between mb-1.5">
                <div className="label-mono">{c.s}</div>
                <div className="label-mono-xs text-ink-mute">{c.p}</div>
              </div>
              <div className="grid grid-cols-6 gap-1">
                {["NONE", "1", "2", "4", "6", "12"].map((n, i) => (
                  <button key={n} className={`label-mono btn-min-sm border-2 ${i === c.a ? "bg-ink text-paper border-ink" : "border-ink/30 hover:border-ink"}`}>
                    {n}
                  </button>
                ))}
              </div>
        ...
        {/* Batch Summary */}
        <DossierCard className="p-5">
          <div className="label-mono text-hazard mb-2">▼ BATCH SUMMARY</div>
          <h3 className="font-display text-xl mb-4">TALLY SHEET</h3>

          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="border-2 border-ink/30 p-3">
              <div className="label-mono-xs text-ink-mute">TOTAL WAX</div>
              <div className="font-display text-xl">218.6 OZ</div>
            </div>
            <div className="border-2 border-hazard/60 p-3">
              <div className="label-mono-xs text-hazard">TOTAL FRAGRANCE</div>
              <div className="font-display text-xl text-hazard">26.2 OZ</div>
            </div>
            <div className="border-2 border-ink/30 p-3">
              <div className="label-mono-xs text-ink-mute">PITCHERS</div>
              <div className="font-display text-xl">4</div>
            </div>
            <div className="border-2 border-ink/30 p-3">
              <div className="label-mono-xs text-ink-mute">CANDLES</div>
              <div className="font-display text-xl">41</div>
            </div>
          </div>

          <div className="bg-paper-deep/40 border-2 border-ink/30 p-3 typewriter text-[12px] space-y-1">
            <div className="label-mono-xs text-ink-mute mb-1">STOCK CHECK</div>
